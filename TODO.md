...
| `P1` | `#104` GPU throughput follow-up hot path | In Progress | `H-001`, `H-003`, `H-004-a` 모두 canonical 회귀로 정리됐고, breakdown probe 결과 kernel의 거의 전부가 `additional_buy`에 몰려 있다 | `H-005-a`를 probe-only slice로 고정하고, `additional_buy` 내부 sub-stage timing을 먼저 넣는다 | [doc](todos/2026_03_13-issue104-gpu-throughput-followup-hotpath.md) |

...
| `P1` | `#104` GPU throughput follow-up hot path | In Progress | `H-001`, `H-003`, `H-004-a` 모두 canonical 회귀로 정리됐고, `H-005-a` / `H-005-b-0` probe 결과 `additional_buy_state_last_trade_update`가 핵심 병목으로 좁혀졌다. `H-005-b` direct-update slice 구현과 회귀 테스트도 완료됐다 | `issue98_perf_measure --kernel-breakdown --runs 1`로 direct-update 이후 `additional_buy_state_last_trade_update_s`와 전체 kernel/wall 변화를 확인한다 | [doc](todos/2026_03_13-issue104-gpu-throughput-followup-hotpath.md) |

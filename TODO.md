...
| `P1` | `#104` GPU throughput follow-up hot path | In Progress | `H-001`, `H-003`, `H-004-a` 모두 canonical 회귀로 정리됐고, `H-005-a` 측정 결과 `additional_buy` 내부의 1차 병목이 `state_update`로 확인됐다 | `H-005-b-0`로 `state_update`를 `final_compact / slot_lookup / position_write / last_trade_update`로 더 잘게 계측한다 | [doc](todos/2026_03_13-issue104-gpu-throughput-followup-hotpath.md) |

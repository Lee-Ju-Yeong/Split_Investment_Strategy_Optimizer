...
def _fetch_and_normalize_short_selling(ticker_code, effective_start, effective_end, wait_slot):
    ...
    try:
        ...
        print(f"[short_selling_collector] HTTPError for {ticker_code} on {effective_start}-{effective_end}: {e}")
        return {"status": "http_error", "error": str(e), "rows": []}
    except json.JSONDecodeError as e:
        print(f"[short_selling_collector] JSONDecodeError for {ticker_code} on {effective_start}-{effective_end}: {e}")
        return {"status": "json_decode_error", "error": str(e), "rows": []}
    except Exception as e:
        print(f"[short_selling_collector] Unexpected error fetching short selling for {ticker_code} on {effective_start}-{effective_end}: {e}")

...
class TestShortlistDerivation(unittest.TestCase):
    ...
    def _write_bundle_manifest(
        ...
        windows: list[dict],
        *,
        optional_fail_budget: int = 0,
        selection_contract_overrides: dict | None = None,
        ...
        if selection_contract_overrides:
            if "family_included_parameters" in selection_contract_overrides:
                selection_contract.pop("family_excluded_parameters", None)
            selection_contract.update(selection_contract_overrides)
        governance_gates = {
        ...
        if governance_overrides:
            governance_gates.update(governance_overrides)
        payload = {
    ...
    def test_derive_shortlist_writes_provenance_and_actual_row_family_rep(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(source_manifest["family_grouping_mode"], "excluded_parameters")
            self.assertEqual(
                source_manifest["family_grouping_parameters"],
                [
                    "max_stocks",
                    "order_investment_ratio",
                    "additional_buy_drop_rate",
    ...
    def test_family_included_parameters_can_force_broader_family_grouping(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            csv_a = tmp / "window_a.csv"
            csv_b = tmp / "window_b.csv"
            ...
            outputs = derive_shortlist(bundle_manifest_path=manifest_path, out_dir=tmp / "out")

            with outputs["shortlist_csv"].open(encoding="utf-8") as handle:
                shortlist_rows = list(csv.DictReader(handle))
            self.assertEqual(len(shortlist_rows), 1)
            self.assertEqual(
                shortlist_rows[0]["family_signature"],
                "max_stocks=20|additional_buy_priority=lowest_order|max_splits_limit=10|max_inactivity_period=63",
            )

            source_manifest = json.loads(outputs["source_manifest"].read_text(encoding="utf-8"))
            self.assertEqual(source_manifest["family_grouping_mode"], "included_parameters")
            self.assertEqual(
                source_manifest["family_grouping_parameters"],
                [
                    "max_stocks",
    ...
    def test_family_grouping_rejects_mixed_included_and_excluded_parameters(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            path = tmp / "window.csv"
            digest = self._write_csv(
            ...
            self._write_bundle_manifest(
                ...
                selection_contract_overrides={
                    "family_included_parameters": ["max_stocks"],
                    "family_excluded_parameters": ["stop_loss_rate"],
                },
            )

            with self.assertRaisesRegex(ValueError, "cannot define both"):
                derive_shortlist(bundle_manifest_path=manifest_path, out_dir=tmp / "out")

    def test_aggregate_minimum_criteria_filters_on_cross_window_mean_cagr(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            csv_a = tmp / "window_a.csv"
            csv_b = tmp / "window_b.csv"
            ...
            self._write_bundle_manifest(
                ...
                selection_contract_overrides={
                    "family_included_parameters": list(HEADER[:8]),
                    "shortlist_size": 5,
                },
                governance_overrides={
                    "aggregate_minimum_criteria": {
                        "cagr_mean_min": 0.14,
                    }
                },
            )
            ...
            with outputs["shortlist_csv"].open(encoding="utf-8") as handle:
                shortlist_rows = list(csv.DictReader(handle))
            self.assertEqual(len(shortlist_rows), 1)
            self.assertEqual(shortlist_rows[0]["order_investment_ratio"], "0.0240")

            source_manifest = json.loads(outputs["source_manifest"].read_text(encoding="utf-8"))
            self.assertEqual(
                source_manifest["aggregate_minimum_criteria"],
                {"cagr_mean_min": 0.14},
            )

    def test_exact_duplicate_stop_loss_resolution_keeps_smallest_stop_loss(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            csv_a = tmp / "window_a.csv"
            csv_b = tmp / "window_b.csv"
            ...
            self.assertEqual(len(shortlist_rows), 1)
            self.assertEqual(shortlist_rows[0]["stop_loss_rate"], "-0.9000")

            source_manifest = json.loads(outputs["source_manifest"].read_text(encoding="utf-8"))
            self.assertTrue(source_manifest["exact_duplicate_resolution"]["enabled"])
            self.assertEqual(source_manifest["exact_duplicate_resolution"]["removed_count"], 1)

    def test_exact_duplicate_inactivity_resolution_keeps_largest_inactivity(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            csv_a = tmp / "window_a.csv"
            csv_b = tmp / "window_b.csv"
            ...
            self.assertEqual(len(shortlist_rows), 1)
            self.assertEqual(shortlist_rows[0]["max_inactivity_period"], "252.0000")

    def test_exact_duplicate_resolution_does_not_merge_different_results(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            csv_a = tmp / "window_a.csv"
            csv_b = tmp / "window_b.csv"

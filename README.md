...
### 2) 설정 파일
- `config.ini`에 MySQL 접속 정보를 설정합니다. (예: host/user/password/database)
...
### 5) 웹 UI 실행
- 실행: `python src/app.py`
- 사전 준비: `DailyStockPrice`, `CalculatedIndicators`, `WeeklyFilteredStocks` 테이블이 채워져 있어야 합니다.
- 결과 파일은 `results_of_single_test/`에 저장됩니다.
...
GPU 가속 백테스팅 최적화기를 위한 개발 로드맵입니다.

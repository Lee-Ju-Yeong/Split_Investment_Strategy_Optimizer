...
- **성능 병목:** 현재 시스템에서 가장 시간이 많이 소요되는 부분은 `backtest_strategy_gpu.py`의 메인 루프, 특히 순차적 자금 차감 로직임. 향후 추가 최적화가 필요하다면 `cupy.RawKernel`을 사용한 CUDA C++ 직접 작성을 고려할 수 있으나, 현재 `CuPy` 수준에서도 충분한 성능을 보임.

...
- `financial_lag_days`
  ...
  - 기본값: `1`
  - 영향: 값이 작으면 반영 속도 향상, 값이 크면 반영 지연 증가
...
END = "2026-02-06"
SCENARIOS = {
    "A_conservative": {"lookback_days": 20, "financial_lag_days": 1, "danger_liquidity": 500_000_000, "prime_liquidity": 2_000_000_000},
    "B_baseline": {"lookback_days": 20, "financial_lag_days": 1, "danger_liquidity": 300_000_000, "prime_liquidity": 1_000_000_000},
    "C_relaxed": {"lookback_days": 20, "financial_lag_days": 1, "danger_liquidity": 100_000_000, "prime_liquidity": 800_000_000},
}
...
- 교차 검토 결론:
  ...
  - `financial_lag_days`는 운영 기본값 `1`로 고정
  - 비교 실험이 필요하면 `--financial-lag-days`를 명시해 별도 시나리오로 실행

튜닝 프로토콜(룩어헤드 방지):

1. Anchored WFO로 train/validation/test 분리
2. 후보 그리드: `lookback {20,30}`, `lag {1}`, `danger {100m,300m,500m}`, `prime {800m,1b,2b}`
